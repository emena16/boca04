...
/* Vamos a recibir el id del producto que se va a eliminar del pedido */
$idProdMedida = $_POST['idProdMedida'];
$idPedido = str_replace(' ','',$_POST['idPedido']);
$response = array(
    'mensaje' => '',
    'status' => 0
);
// Vamos a eliminar el producto del pedido
$query = "DELETE FROM pdo_prod WHERE id_pdo = '$idPedido' AND id_prod = $idProdMedida";
if(!$result = mysqli_query($conexion_bd,$query)){
    $response['status'] = 1;
    $response['mensaje'] = "Error al eliminar el producto del pedido";
}
