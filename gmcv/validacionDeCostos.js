 ...
 */
function ajustesPactadoExtra(
  sumaP2S2, // Suma de las celdas entre P2 y S2
  g2,       // Valor en G2
  f2,       // Valor en F2
  m2,       // Valor en M2
  n2        // Valor en N2
) {
  // Primera condición: la suma de P2 a S2 es cero
  const condition1 = (sumaP2S2 === 0);

  // Segunda condición: G2 es mayor que F2
  const condition2 = (g2 > f2);

  // Si ambas condiciones son verdaderas
  if (condition1 && condition2) {
    const diffGF = g2 - f2; // Diferencia entre G2 y F2
    const diffMN = m2 - n2; // Diferencia entre M2 y N2
    return diffGF * diffMN; // Producto de las diferencias
  }

  // Si alguna condición es falsa, retorna 0
  return 0;
}



function ajustesIngresoExtra(
  sumPListaYAjustesDescAnt, // Suma de los elementos de P a S
  sumAjusteDescPost,        // Suma de los elementos de T a U
  costoUnitarioFactura,     // G2
  costoIngresoPactado,      // K2
  canFacturada,             // M2
  cantRechazada             // N2
) {
  // Condiciones para determinar si se aplican ajustes
  const condition1 = (sumPListaYAjustesDescAnt === 0 && sumAjusteDescPost === 0);
  const condition2 = (costoUnitarioFactura > costoIngresoPactado);

  // Si las dos condiciones son verdaderas
  if (condition1 && condition2) {
    const diffCosto = costoUnitarioFactura - costoIngresoPactado; // Diferencia entre costo unitario y costo pactado
    const diffCantidad = canFacturada - cantRechazada;            // Diferencia entre cantidad facturada y rechazada
    return diffCosto * diffCantidad; // Retorna el producto de las diferencias
  }
...
function exportarFacturaAExcel(factura) {
  ...
  switch (model.form.ajuste.val) {
    ...
    case '2':
      console.log("Calculando Contra Precio Lista");
    ...
    case '1':
      console.log("Calculando Contra Costo Ingreso");
      var csv = 'Nombre del Producto, Precio Lista,'
      ...
      //Creamos variables para totalizar las columnas de diferenctes partes del documento
      let totalCantidadFacturada = 0;
      let totalCantidadRechazada = 0;
      let totalSubtotalFactura = 0;
      let totalAjusteRechazo = 0;
      let totalSubtotalDespuesRechazos = 0;
      let totalAjustePrecioLista = 0;
      let totalDesctosAntes = 0
      let totalDesctosDespues = 0;
      let totalAjustePactadoExtra = 0;
      let totalAjusteIngresoExtra = 0;
      ...
      csv += 'Costo Ingreso Pactado, Costo Ingreso Factura, Cantidad Facturada, Subtotal Factura, Cantidad Rechazada, Ajuste Rechazo, Subtotal despues rechazos, Ajuste Precio Lista,' + nombresDescuentosAntes + nombresDescuentosDespues + '  Ajuste Pactado Extra, Ajuste Ingreso Extra, Ajuste Total\n'
      ...
      factura.items.forEach(item => {
        ...
        //Calculamos los descuentos antes del costoPactado
        let sumaDescuentosAntes = 0;
        item.desc.filter(desc => desc.posteriorCP == 0).forEach(desc => {
          csv += (desc.descuento / 100 ? desc.descuento / 100 : '0') + ',';
          sumaDescuentosAntes += desc.descuento ? Number(desc.descuento) : 0;
        ...
        let ajustePactadoExtra = ajustesPactadoExtra(sumaAjustesDescuentosAntes, item.costoReal, item.costoPactado, item.cantidad, item.cantidad_rechazada);
        let ajusteIngresoExtra = ajustesIngresoExtra(sumaAjustesDescuentosAntes, sumaAjustesDescuentosDespues, item.costoReal, item.costoPactado, item.cantidad, item.cantidad_rechazada);
        //Esta parte era el antiguo calculo de ajusteIngresoExtra
        let costoIngresoPactado = item.costoPactado * (1 - sumaDescuentosPost / 100)
        ajusteRechazo = item.costoReal * item.cantidad_rechazada;
        //Obtenemos el subtotal real de la factura descontando los rechazados
        let subTotalRecibido = item.costoFacturaBruto.toFixed(4) - ajusteRechazo.toFixed(4)

        // let cantidadFacturada = item.cantidad - item.cantidad_rechazada;
        let cantidadFacturada = item.cantidad;
        ...
        csv += (costoIngresoPactado ? costoIngresoPactado.toFixed(4) : '') + ','
          ...
          //Cantidad Facturada
          + (cantidadFacturada ? cantidadFacturada : '0') + ','
          // Subtotal Factura
          + (item.costoFacturaBruto ? item.costoFacturaBruto.toFixed(4) : '') + ','
          // Cantidad Rechazada
          + (item.cantidad_rechazada ? item.cantidad_rechazada : '') + ','
          ...
          //Agremamos el Subtotal despues rechazos
          + (subTotalRecibido.toFixed(4)) + ','
          //Calculamos el ajuste de precio lista
          + (ajustePrecioLista.toFixed(4)) + ','
        ...
        //Sumamos para totalizar cada columna
        totalCantidadFacturada += cantidadFacturada;
        totalCantidadRechazada += parseFloat(item.cantidad_rechazada);
        totalSubtotalFactura += item.costoFacturaBruto;
        totalAjusteRechazo += parseFloat(ajusteRechazo);
        totalSubtotalDespuesRechazos += subTotalRecibido;
        totalAjustePrecioLista += ajustePrecioLista;
        totalDesctosAntes += sumaAjustesDescuentosAntes;
        totalDesctosDespues += sumaAjustesDescuentosDespues;
        totalAjustePactadoExtra += ajustePactadoExtra;
        totalAjusteIngresoExtra += ajusteIngresoExtra;
        ...
        //Escribimos los totales de cada columna para finalizar el documento
        csv += '\nTotales:,,,,,,,,,' + totalCantidadFacturada + ',' + totalSubtotalFactura.toFixed(4) + ',' + totalCantidadRechazada + ',' + totalAjusteRechazo.toFixed(4) + ',' + totalSubtotalDespuesRechazos.toFixed(4) + ',' + totalAjustePrecioLista.toFixed(4) + ',' + totalDesctosAntes.toFixed(4) + ',' + totalDesctosDespues.toFixed(4) + ',' + totalAjustePactadoExtra.toFixed(4) + ',' + totalAjusteIngresoExtra.toFixed(4) + ',' + ajusteTotal.toFixed(4) + '\n'
    ...
    case '0': // Caso: Contra Costo Pactado
      console.log("Calculando Contra Costo Pactado");
      var csv = 'Nombre del Producto, Precio Lista,'
      ...
      factura.items.forEach(item => {
        ...
        // Calculamos ajuste pactado extra
        let ajustePactadoExt = ajustesPactadoExtra(sumaAjustesDttosAntes, item.costoReal, item.costoPactado, item.cantidad, item.cantidad_rechazada);
    ...
    default:
      console.log("Hubo un error al seleccionar el tipo de ajuste, seleccionado por DEFAULT el valor es: " + model.form.ajuste.val);
      var csv = 'Nombre del Producto, Precio Lista,'
      factura.descuento.items.filter(desc => desc.posteriorCP == 0).forEach(desc => csv += ' ' + desc.nombre + '(%),')
      csv += ' Precio Pactado, Costo Unitario Factura, Diff %,'
      factura.descuento.items.filter(desc => desc.posteriorCP == 1).forEach(desc => csv += ' ' + desc.nombre + '(%),')
